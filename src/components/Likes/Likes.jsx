...
const Likes = ({ article, loggedIn, token }) => {
  ...
  const likeColor = isLiked && loggedIn ? 'red' : 'black';
  const likeStyle = loggedIn ? 'active-like-button' : 'non-active-like-button';
  const switchLike = () => {
    if (!loggedIn) {
      return;
    }
  ...
  return (
    <div className="likes-container">
      <div className={`likes-icon ${likeStyle}`} onClick={switchLike}>
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
          <g clipPath="url(#clip0_73408_29)">
            <path
      ...
      {likeError && (
        <div className="sign-up-error-container">
          <p className="sign-up-error-message">Error! Try to refresh page!</p>
        </div>
      )}
    </div>
  );
